...
# 🔹 Configuração correta de logging (removida duplicação)
logging.basicConfig(
    level=logging.DEBUG,
...
class BaseTest:
    ...
    def _build_url(self, endpoint):
        """Metodo auxiliar para criar URLs completas."""
        return f"{self.base_url}{endpoint}"

    def _build_headers(self, headers=None, include_auth=False):
        """Metodo auxiliar para incluir sempre o token automaticamente."""
    ...
    def check_status_code(response, expected_status_code):
        logging.info(f"🔹 Response completo: {response.text}")
        logging.info("🔹 Status Code Esperado: %s | Recebido: %s", expected_status_code, response.status_code)
